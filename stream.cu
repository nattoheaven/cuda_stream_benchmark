...
static __device__ float2	a[N+OFFSET],
				b[N+OFFSET],
				c[N+OFFSET];
...
static __global__ void STREAM_Init()
{
  ...
  if (j < N) {
    a[j] = make_float2(1.0f, 1.0f);
    b[j] = make_float2(2.0f, 2.0f);
    c[j] = make_float2(0.0f, 0.0f);
  }
}
...
static __global__ void STREAM_Test()
{
  int j = ((blockIdx.z * gridDim.y + blockIdx.y) * gridDim.x + blockIdx.x) *
    blockDim.x + threadIdx.x;
  if (j < N) {
    a[j] = make_float2(2.0E0f * a[j].x, 2.0E0f * a[j].y);
  }
}
...
static __global__ void STREAM_Scale(float scalar)
{
  int j = ((blockIdx.z * gridDim.y + blockIdx.y) * gridDim.x + blockIdx.x) *
    blockDim.x + threadIdx.x;
  if (j < N) {
    b[j] = make_float2(scalar * c[j].x, scalar * c[j].y);
  }
}
...
static __global__ void STREAM_Add()
{
  int j = ((blockIdx.z * gridDim.y + blockIdx.y) * gridDim.x + blockIdx.x) *
    blockDim.x + threadIdx.x;
  if (j < N) {
    c[j] = make_float2(a[j].x + b[j].x, a[j].y + b[j].y);
  }
}
...
static __global__ void STREAM_Triad(float scalar)
{
  int j = ((blockIdx.z * gridDim.y + blockIdx.y) * gridDim.x + blockIdx.x) *
    blockDim.x + threadIdx.x;
  if (j < N) {
    a[j] = make_float2(b[j].x + scalar * c[j].x, b[j].y + scalar * c[j].y);
  }
}

static __device__ void STREAM_Sum_sub(double *sum, const float2 *a,
				      int j, double *shared)
{
  double x;
  if (j < N) {
